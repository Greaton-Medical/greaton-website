...
(() => {
  ...
  function initSection1() {
    ...
    // Animate player between two progress values
    ...
    // Click: expand → play, or pause → collapse back
    function s1ExpandAndPlay() {
      ...
      if (!sec._s1ClickExpanded) {
        ...
        // Was paused mid-scroll — just resume
        v.play().catch(() => {});
        sec._s1ClickExpanded = true;
      }
    }
    ...
    // Pause when player leaves viewport, resume when it returns
    if (player) {
    ...
    // Expose so updateSection1 can trigger first play on scroll
  ...
  function updateSection1() {
    ...
    // Never shrink player if user already clicked to expand
